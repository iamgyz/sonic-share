...
/**
 * Given a frequency, convert to the corresponding bit.
 */
SonicCoder.prototype.freqToBit = function(freq) {
    // If the frequency is out of the range.
    if (this.freqMin < freq && freq < this.freqMax) {
        //check if in start signal
        if(freq > this.freqMin && freq < this.freqMin+200){
            return this.startChar;
        }
        //check if bit is 0
        else if(freq >= this.freqMin+200 && freq < this.freqMin+500){
            return '0';
        }
        else if(freq >= this.freqMin+500 && freq < this.freqMin+800){
            return '1';
        }
        //else end signal
        else{
...
SonicServer.prototype.loop = function() {
  ...
  if (freq) {
    var char = this.coder.freqToBit(freq);
...
SonicServer.prototype.analysePeaks = function() {
  ...
  if (this.state == State.IDLE) {
    ...
    if (char == this.coder.endChar) {
      var message = this.getWholeMessage();
      this.state = State.IDLE;
      this.fire_(this.callbacks.message,message);
      this.peakTimes.clear();
      this.peakHistory.clear();
...
SonicServer.prototype.getWholeMessage = function(){
    //因為總共有8bit+start = 10
    //先算出有幾個chunk，再把每個chunk理的1 0相加，看相加的結果很大還很小
    var data = [];
    for(var i=0;i<this.peakHistory.length();i++){
        _char = this.peakHistory.get(i);
        if(_char == '^'){
            continue;
        }else if(_char== '$'){
            break;
    ...
    for(var i=0;i<8;i++){
        sum[i] = 0;
        for(var j= i*chunks; j< i*chunks+chunks && j< data.length ; j++){
            sum[i] = sum[i] + data[j];
        }//for
    }//for
    if (this.debug)
...
SonicServer.prototype.getLastRun = function() {
  //console.log("in getLastRun",this.peakHistory);
...
},{"./lib/sonic-coder.js":2,"./lib/sonic-server.js":3,"./lib/sonic-socket.js":4,"jquery":6}],6:[function(require,module,exports){
/*!
